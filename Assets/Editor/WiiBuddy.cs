...
public class WiiBuddy : Editor{
	...
	private void WakeUp()
	{
		...
		//Debug.Log ("TEST STRUCT SIZE          "+test ());
	...
	public override void OnInspectorGUI (){
		...
		{	
			...
			if(currentStates[i].active || isVirtual)
			{
				...
				else if(currentStates[i].expType == 7)//wii u pro controller
				{
					panel = EditorGUILayout.Foldout(showExpPanel[i], "Wii U Pro");
					...
					if(showExpPanel[i])
					{
						...
						EditorGUILayout.LabelField("Battery unknown...");
						//currentStates[i].battery = EditorGUILayout.FloatField("Battery unknown",currentStates[i].battery,GUILayout.MaxWidth(150));
						//currentStates[i].battery = Mathf.Clamp(currentStates[i].battery,0,1);

						Vector2 proUStickL = new Vector2(currentStates[i].expFloat1,currentStates[i].expFloat2);
						Vector2 proUStickR = new Vector2(currentStates[i].expFloat3,currentStates[i].expFloat4);
						if(!isVirtual)
						{
							proUStickL = Wii.GetWiiUProAnalogStick(i,"LEFT");
							proUStickR = Wii.GetWiiUProAnalogStick(i,"RIGHT");
							proUStickL.x = ((float)(int)(proUStickL.x*1000))/1000f;
							proUStickL.y = ((float)(int)(proUStickL.y*1000))/1000f;
							proUStickR.x = ((float)(int)(proUStickR.x*1000))/1000f;
							proUStickR.y = ((float)(int)(proUStickR.y*1000))/1000f;
						}
						Vector2 newStick = EditorGUILayout.Vector2Field("Wii U Pro Stick Left",proUStickL,GUILayout.MaxWidth(300));
						if(isVirtual && newStick != proUStickL)
						{
						...
						newStick = EditorGUILayout.Vector2Field("Wii U Pro Stick Right",proUStickR,GUILayout.MaxWidth(300));
						if(isVirtual && newStick != proUStickR)
						{
							newStick.x = Mathf.Clamp(newStick.x,-1,1);
							newStick.y = Mathf.Clamp(newStick.y,-1,1);
						...
						EditorGUILayout.BeginHorizontal();
						if (GUILayout.Button ("Recalibrate Analog Sticks", GUILayout.MaxWidth (150))) {
							Wii.CalibrateWiiUPro(i);
						}
						EditorGUILayout.EndHorizontal();
						EditorGUILayout.BeginHorizontal();
						...
						currentStates[i].expButton11 = GUILayout.Toggle(currentStates[i].expButton11,
						...
						EditorGUILayout.BeginHorizontal();
						currentStates[i].expFloat5 = GUILayout.Toggle(currentStates[i].expFloat5!=0,
						                                                "L3",GUILayout.MaxWidth(35))?1:0;
						currentStates[i].expFloat6 = GUILayout.Toggle(currentStates[i].expFloat6!=0,
						                                                "R3",GUILayout.MaxWidth(35))?1:0;

						currentStates[i].expButton14 = GUILayout.Toggle(currentStates[i].expButton14,
						                                                "L",GUILayout.MaxWidth(25));

						currentStates[i].expButton15 = GUILayout.Toggle(currentStates[i].expButton15,
						                                                "R",GUILayout.MaxWidth(25));
